...
class MainWindow(ctk.CTk):
    ...
    def setup_left_panel(self):
        ...
        title_label = ctk.CTkLabel(header_frame, text=t("title_main"), font=ctk.CTkFont(size=20, weight="bold"))
        title_label.grid(row=0, column=0, sticky="w")
        
        settings_btn = ctk.CTkButton(header_frame, text=t("btn_settings"), width=80, command=self.open_settings)
        ...
        ctk.CTkLabel(batch_frame, text=t("lbl_batch_count")).pack(side="left", padx=5)
        ...
        self.freeu_switch = ctk.CTkSwitch(freeu_frame, text=t("lbl_freeu_enable"), variable=self.freeu_var)
        ...
        self.queue_frame = ctk.CTkScrollableFrame(self.left_frame, label_text=t("lbl_queue"))
        ...
        ctk.CTkButton(btn_frame, text=t("btn_add_queue_wizard"), command=self.add_queue_wizard).grid(row=0, column=0, padx=5, sticky="ew")
        ctk.CTkButton(btn_frame, text=t("btn_add_empty_queue"), command=self.add_empty_queue).grid(row=0, column=1, padx=5, sticky="ew")
    ...
    def add_queue_card(self, initial_text="", save=True):
        ...
        ctk.CTkLabel(header, text=t("lbl_queue_item"), font=ctk.CTkFont(weight="bold")).pack(side="left")
    ...
    def setup_right_panel(self):
        ...
        self.preset_requeue_frame = ctk.CTkFrame(self.right_frame, fg_color="transparent")
        self.preset_requeue_frame.grid(row=2, column=0, padx=10, pady=5, sticky="ew")
        self.preset_requeue_frame.grid_columnconfigure((0, 1), weight=1)
        
        save_preset_btn = ctk.CTkButton(self.preset_requeue_frame, text=t("btn_save_preset"), command=self.save_preset)
    ...
    def start_generation(self):
        ...
        # Update UI state
        self.generate_btn.configure(text=t("btn_cancel"), fg_color="#C62828", hover_color="#b71c1c")
    ...
    def on_gen_start(self, item):
        self.after(0, lambda: self.status_label.configure(text=t("status_generating")))

    def on_gen_progress(self, info):
        progress = info.get("progress", 0)
        self.after(0, lambda: [
            self.progress_bar.set(progress),
            self.status_label.configure(text=t("status_generating_pct", pct=int(progress*100)))
        ])
    ...
    def on_gen_error(self, err_msg):
        ...
        self.after(0, lambda: [
            messagebox.showerror(t("msg_error"), err_msg),
            self.on_gen_complete()
        ])

    def on_gen_complete(self):
        self.after(0, lambda: [
            self.generate_btn.configure(text=t("btn_generate"), fg_color="green", hover_color="darkgreen"),
            self.status_label.configure(text=t("lbl_status_idle")),
            self.progress_bar.set(0.0)
        ])
