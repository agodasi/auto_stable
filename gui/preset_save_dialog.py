...
class PresetSaveDialog(ctk.CTkToplevel):
    def __init__(self, parent, current_prompt, on_save):
        ...
        self.title("Save Preset from Current Prompt")
        ...
        ctk.CTkLabel(self.bottom_frame, text="Current Prompt (Copy & Paste to the forms above):", font=ctk.CTkFont(weight="bold")).pack(anchor="w", pady=5)
        ...
        ctk.CTkLabel(self.step1_frame, text="Step 1: Save as Situation Preset", font=ctk.CTkFont(size=16)).pack(pady=5)
        self.sit_name_entry = ctk.CTkEntry(self.step1_frame, placeholder_text="Preset Name (New or Overwrite)", width=400)
        ...
        ctk.CTkButton(btn_frame1, text="Skip", command=self.go_step2, width=100).pack(side="left", padx=10)
        ctk.CTkButton(btn_frame1, text="Next", command=self.go_step2, width=100).pack(side="left", padx=10)
        
        # Step 2: Character
        self.step2_frame = ctk.CTkFrame(self.top_frame, fg_color="transparent")
        
        ctk.CTkLabel(self.step2_frame, text="Step 2: Save as Character Preset", font=ctk.CTkFont(size=16)).pack(pady=5)
        self.char_name_entry = ctk.CTkEntry(self.step2_frame, placeholder_text="Preset Name (New or Overwrite)", width=400)
        ...
        ctk.CTkButton(btn_frame2, text="Back", command=self.go_step1, width=100).pack(side="left", padx=10)
        ctk.CTkButton(btn_frame2, text="Save", command=self.save_and_close, width=100).pack(side="left", padx=10)
