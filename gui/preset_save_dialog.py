...
class PresetSaveDialog(ctk.CTkToplevel):
    def __init__(self, parent, current_prompt, on_save):
        ...
        self.title(t("title_preset_save"))
        ...
        ctk.CTkLabel(self.step1_frame, text=t("step1_save_sit"), font=ctk.CTkFont(size=16)).pack(pady=5)
        self.sit_name_entry = ctk.CTkEntry(self.step1_frame, placeholder_text=t("ph_preset_name"), width=400)
        ...
        ctk.CTkButton(btn_frame1, text=t("btn_skip"), command=self.go_step2, width=100).pack(side="left", padx=10)
        ctk.CTkButton(btn_frame1, text=t("btn_next"), command=self.go_step2, width=100).pack(side="left", padx=10)
        
        # Step 2: Character
        self.step2_frame = ctk.CTkFrame(self.top_frame, fg_color="transparent")
        
        ctk.CTkLabel(self.step2_frame, text=t("step2_save_char"), font=ctk.CTkFont(size=16)).pack(pady=5)
        self.char_name_entry = ctk.CTkEntry(self.step2_frame, placeholder_text=t("ph_preset_name"), width=400)
        ...
        ctk.CTkButton(btn_frame2, text=t("btn_back"), command=self.go_step1, width=100).pack(side="left", padx=10)
        ctk.CTkButton(btn_frame2, text=t("btn_save"), command=self.save_and_close, width=100).pack(side="left", padx=10)
